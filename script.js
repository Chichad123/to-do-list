...
function appendTicket(ticketInfoObject) {
  ...
  let ticketEditBtn=ticketDiv.querySelector(".ticket-edit");
  ticketEditBtn.addEventListener("click",function(e){
      EditTicketModal(ticketInfoObject);


  });
  function EditTicketModal(ticketInfoObject)
  {
    let { ticketFilter, ticketValue, ticketId } = ticketInfoObject;
    let ticketmodaledit=document.createElement("div");
    ticketmodaledit.classList.add("ticket-modal");
    ticketmodaledit.innerHTML = `
    ...
    for(let i=0;i<allTickets.length;i++)
    {
      if(allTickets[i].ticketFilter==ticketFilter)
      {
        e.target.classList.add("selected-filter");
        break;
      }
